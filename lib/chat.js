...
class Chat {
  ...
  start () {
    ...
    this.client.on('message', (target, context, msg, self) => {
      ...
      const terminalColor = color.getTerminalColor(context['color'] || name)
      const nameWithColor = safeColors[terminalColor](name).padEnd(25, " ")
      terminal.log(`${safeColors['gray'](getTimeString())} ${nameWithColor}: ${msg}`)
    })
    this.client.connect()
    return this
  }
